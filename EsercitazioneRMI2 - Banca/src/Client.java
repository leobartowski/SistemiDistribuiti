...
public class Client {
    ...
    public static void handleLoginOrSignup() {
        try {
            InterfaceBank serverForLogin = (InterfaceBank) Naming.lookup("//localhost:8080/bank_handle_login");
            final String password = MyUtility.askForNewPassword();
            final float startingBalance = MyUtility.askForStartingBalance();
            serverForLogin.createAccount(startingBalance, password);
            System.out.println("New account successful created");
//            Naming.unbind("//localhost:8080/bank_handle_login");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
